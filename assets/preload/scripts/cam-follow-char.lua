...
local xx = 720; -- Code to change the position of the camera to the left or right for your opponent, Less = Left (They can be negative numbers), More = Right
...
local xx2 = 1220; -- Same code as above, but for boyfriend left, right
local yy2 = 850; -- Same code as above, but for boyfriend up, down
...
local ofs = 75; -- Code to adjust the intensity with which the camera moves, the more numbers, the more intense, and the fewer numbers, less intense
...
function onUpdatePost()
    middlescroll = true
    for i = 0,3 do
        noteTweenAlpha(i, i, 0, 0.1)
    end
end

function onMoveCamera(f)
    if f == 'boyfriend' then
    triggerEvent('Add Camera Zoom',0.75,0)
    end
end